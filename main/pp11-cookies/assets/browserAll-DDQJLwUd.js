...
function G(a) {
  ...
  var n = ie(t),
    s = {
      ...
      amazon: { phone: n(E), tablet: !n(E) && n(A), device: n(E) || n(A) },
      android: {
        phone: (!n(m) && n(E)) || (!n(m) && n(x)),
        tablet: !n(m) && !n(E) && !n(x) && (n(A) || n($)),
        device: (!n(m) && (n(E) || n(A) || n(x) || n($))) || n(/\bokhttp\b/i),
      },
      windows: { phone: n(m), tablet: n(X), device: n(m) || n(X) },
      other: {
        blackberry: n(F),
        blackberry10: n(Y),
        opera: n(N),
  ...
  L = class V {
    ...
    _activate() {
      ...
      (this._isActive = !0),
        this._div ||
          ...
          (this._div.style.width = `${O}px`),
          (this._div.style.height = `${O}px`),
          (this._div.style.position = "absolute"),
          (this._div.style.top = `${re}px`),
          (this._div.style.left = `${ae}px`),
          (this._div.style.zIndex = W.toString()),
    ...
    init(e) {
      ...
      (this.debug = i.accessibilityOptions.debug),
        ...
        (this._deactivateOnMouseMove =
          i.accessibilityOptions.deactivateOnMouseMove),
    ...
    postrender() {
      ...
      for (let i = this._children.length - 1; i >= 0; i--) {
        ...
        t.has(i) ||
          ...
          Q(this._children, i, 1));
      }
      if (this._renderer.renderingToScreen) {
        const { x: i, y: n, width: s, height: o } = this._renderer.screen,
          r = this._div;
        (r.style.left = `${i}px`),
          (r.style.top = `${n}px`),
          (r.style.width = `${s}px`),
      ...
      for (let i = 0; i < this._children.length; i++) {
        ...
        if (n.hitArea) {
          const r = n.worldTransform,
            ...
            d = this._renderer.resolution;
          (s.style.left = `${(r.tx + o.x * r.a) * l}px`),
            (s.style.top = `${(r.ty + o.y * r.d) * d}px`),
            (s.style.width = `${o.width * r.a * l}px`),
            (s.style.height = `${o.height * r.d * d}px`);
        } else {
          this._capHitArea(o);
          const r = this._renderer.resolution,
            l = this._renderer.resolution;
          (s.style.left = `${o.x * r}px`),
            (s.style.top = `${o.y * l}px`),
            (s.style.width = `${o.width * r}px`),
    ...
    _dispatchEvent(e, t) {
      const { container: i } = e.target,
        ...
        s = Object.assign(new M(n), { target: i });
...
L.extension = { type: [b.WebGLSystem, b.WebGPUSystem], name: "accessibility" };
L.defaultOptions = {
  enabledByDefault: !1,
  debug: !1,
  activateOnTab: !0,
...
class q {
  ...
  postrender() {
    ...
    this._domElement.parentNode !== t.parentNode &&
      ((s = t.parentNode) == null || s.appendChild(this._domElement));
    const i = (parseFloat(t.style.width) / t.width) * this._renderer.resolution,
      n = (parseFloat(t.style.height) / t.height) * this._renderer.resolution;
    this._domElement.style.transform = `translate(${t.offsetLeft}px, ${t.offsetTop}px) scale(${i}, ${n})`;
    for (let o = 0; o < e.length; o++) {
      const r = e[o],
        l = r.element;
      if (!r.parent || r.globalDisplayStatus < 7)
        l == null || l.remove(), e.splice(o, 1), o--;
      else {
        this._domElement.contains(l) ||
          ((l.style.position = "absolute"),
          (l.style.pointerEvents = "auto"),
          this._domElement.appendChild(l));
        const d = r.worldTransform,
          p = r._anchor,
          c = r.width * p.x,
          h = r.height * p.y;
        (l.style.transformOrigin = `${c}px ${h}px`),
          (l.style.transform = `matrix(${d.a}, ${d.b}, ${d.c}, ${d.d}, ${
            d.tx - c
          }, ${d.ty - h})`),
          (l.style.opacity = r.groupAlpha.toString());
      }
    }
  }
  destroy() {
...
class pe {
  ...
  addTickerListener() {
    this._tickerAdded ||
      !this.domElement ||
      (C.system.add(this._tickerUpdate, this, ee.INTERACTION),
      (this._tickerAdded = !0));
  }
  removeTickerListener() {
    this._tickerAdded &&
      (C.system.remove(this._tickerUpdate, this), (this._tickerAdded = !1));
  }
  pointerMoved() {
    this._didMove = !0;
  }
...
class D extends M {
  ...
  initMouseEvent(e, t, i, n, s, o, r, l, d, p, c, h, v, u, be) {
...
class me {
  ...
  hitTestMoveRecursive(e, t, i, n, s, o = !1) {
    ...
    if (
      ((e.eventMode === "dynamic" || t === "dynamic") && (_.pauseUpdate = !1),
      e.interactiveChildren && e.children)
    ) {
      const p = e.children;
      for (let c = p.length - 1; c >= 0; c--) {
        const h = p[c],
          v = this.hitTestMoveRecursive(
            h,
        ...
        if (v) {
          ...
          (v.length > 0 || u) &&
            (u && this._allInteractiveElements.push(e), v.push(e)),
  ...
  hitPruneFn(e, t) {
    if (
      e.hitArea &&
      (e.worldTransform.applyInverse(t, P), !e.hitArea.contains(P.x, P.y))
    )
      return !0;
  ...
  hitTestFn(e, t) {
    return e.hitArea
      ...
      ? (e.worldTransform.applyInverse(t, P), e.containsPoint(P))
      : !1;
  }
  notifyTarget(e, t) {
    var s, o;
  ...
  mapPointerMove(e) {
    ...
    if (
      ((l = n.overTargets) == null ? void 0 : l.length) > 0 &&
      s !== t.target
    ) {
      const p = e.type === "mousemove" ? "mouseout" : "pointerout",
        c = this.createPointerEvent(e, p, s);
      if (
        ...
        for (
          ...
          h.target && !t.composedPath().includes(h.target);
    ...
    if (s !== t.target) {
      ...
      if (!h || h === this.rootTarget.parent) {
        const u = this.clonePointerEvent(t, "pointerenter");
        for (
          u.eventPhase = u.AT_TARGET;
          u.target && u.target !== s && u.target !== this.rootTarget.parent;

        )
          (u.currentTarget = u.target),
            this.notifyTarget(u),
            i && this.notifyTarget(u, "mouseenter"),
  ...
  mapPointerUp(e) {
    ...
    if (o) {
      ...
      if (
        (l.target === r.target && t - l.timeStamp < 200
          ? ++l.clickCount
          : (l.clickCount = 1),
        (l.target = r.target),
        (l.timeStamp = t),
  ...
  createPointerEvent(e, t, i) {
    ...
    return (
      ...
      (n.originalEvent = e),
      (n.target =
        i ?? this.hitTest(n.global.x, n.global.y) ?? this._hitElements[0]),
  ...
  _notifyListeners(e, t) {
    ...
    if (i)
      if ("fn" in i)
        ...
        for (
          let n = 0, s = i.length;
          n < s && !e.propagationImmediatelyStopped;
          n++
        )
  ...
  B = class k {
    ...
    init(e) {
      ...
      this.setTargetElement(t),
        (this.resolution = i),
        (k._defaultEventMode = e.eventMode ?? "passive"),
...
w.mixin(Z, _e);
w.add(q);
//# sourceMappingURL=browserAll-DDQJLwUd.js.map
